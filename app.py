...
def generate_problem_set(api_key, school_level, grade, topic, difficulty_level, question_type):
    ...
    if school_level == "중학교":
        ...
        if grade == "1학년":
            ...
            difficulty_guide = "CSAT (SuNeung) Level. Length: 350-500 words. Vocab: 5000-8000 words level. Highly abstract/academic. Requires deep inference. Mimic 'Killer Question' complexity."
    ...
    if "종합" in question_type:
        ...
        - Create exactly 5 multiple-choice questions (5 options each) with VARIED formats (Avoid too many blanks):
          ...
          - Q4: Vocabulary Appropriateness. **Highlight 5 words in the passage as (a)~(e)**. The options MUST include the highlighted word.
        ...
        - Create exactly 2 multiple-choice questions (Standard CSAT Q41-42 Format):
          ...
          - Q2: Vocabulary appropriateness in context (문맥상 낱말의 쓰임)
        ...
        - Create exactly 3 multiple-choice questions (Standard CSAT Q43-45 Format):
          ...
          - Q2: Pointing Inference (Targeting pronouns a,b,c,d,e).
    ...
    **Requirements**:
    ...
    3. **CRITICAL - Handling Blanks/Context**:
       ...
       - Example Question Text: "다음 빈칸에 들어갈 말로 가장 적절한 것은? \n\n'He showed great _______ and won many championships.'"
    ...
    **Output Format**:
    ...
    try:
        ...
        # Remove trailing commas which cause JSON errors (Gemini sometimes adds them)
        ...
        if "404" in error_msg:
            try:
                # Attempt to list available models to debug the 404 error
                available_models = []
...
# --- Settings UI (Mobile Friendly: Top of Main Area) ---
# Disable inputs while generating to prevent accidental interruption
...
with st.expander("⚙️ 설정 및 주제 선택 (Settings)", expanded=True):
    ...
    # Question Type Selection (Full Width for better alignment)
        ...
        with st.spinner("문제를 생성하고 있습니다... (약 10~20초 소요)"):
            try:
                ...
                if "error" in result:
                    ...
                    st.session_state.graded = False # Reset grading state
            except Exception as e:
...
if st.session_state.generated_content:
    ...
    st.divider()
    # st.subheader("✍️ 문제 (Questions)") # Removed header
    ...
    with st.form("quiz_form"):
        for idx, q in enumerate(questions):
            ...
            # Bold only the question number to avoid markdown issues with multi-line questions
    ...
    if st.session_state.graded:
        ...
        for idx, q in enumerate(questions):
            correct_answer_raw = str(q.get('answer')).strip() # Usually "3" or "3. Option"
            ...
            # Logic to handle if API returns "3" or "3."
        ...
        # Show Detailed Explanations only after grading
        st.divider()
        ...
        # Vocabulary Section
        st.divider()
        with st.expander("📚 주요 어휘 및 숙어 정리 (Vocabulary)"):
            vocab_list = result.get('vocabulary', [])
            if vocab_list:
                for v in vocab_list:
                    st.markdown(f"- **{v.get('word')}**: {v.get('meaning')}")
            else:
